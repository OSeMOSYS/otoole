...
.. NOTE::
    ...
    from the ``simplicity/`` directory. Also note, that while ``otoole`` does not
    depend on any solver, these examples will require you to install the free and
    open source solvers GLPK_ and CBC_. Installation instructions are described
    in the `Solver Setup`_ section.
